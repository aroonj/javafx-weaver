...
/**
 * This class helps to create a generic {@link FxControllerAndView} bean factory
 * that can be used for direct injection of {@link FxControllerAndView}
 * instances into Spring components, based on generic type inspection.
 * <p/>
 * To use, define a {@link FxControllerAndView} bean factory method like shown
 * below. <strong>It is absolutely crucial to define it in prototype scope,
 * since every injection point has to be investigated for its declared generic
 * types!</strong>
 * 
 * <pre>
 * &#64;Bean
 ...
 * 		InjectionPointLazyFxControllerAndViewResolver controllerAndViewResolver, InjectionPoint injectionPoint) {
 * 	return controllerAndViewResolver.resolve(injectionPoint);
 * }
 * </pre>
 * <p>
 * Based on this configuration, a {@link LazyFxControllerAndView} instance can
 * be injected directly into a component:
 * 
 * <pre>
 * &#64;Component
...
public class InjectionPointLazyFxControllerAndViewResolver {
	...
	/**
	 * Resolve generic type classes of a {@link FxControllerAndView}
	 * {@link InjectionPoint} and return a {@link LazyFxControllerAndView} embedding
	 * the {@link FxWeaver#load(Class)} method for instance creation.
	 *
	 * @param injectionPoint the actual injection point for the
	 *                       {@link FxControllerAndView} to inject
	 * @throws IllegalArgumentException when types could not be resolved from the
	...
	private ResolvableType findResolvableType(InjectionPoint injectionPoint) {
		return Optional.ofNullable(injectionPoint.getMethodParameter()).map(ResolvableType::forMethodParameter)
				// TODO: Refactor the following to use .or() when dropping Java 8 support
				.orElse(Optional.ofNullable(injectionPoint.getField()).map(ResolvableType::forField).orElse(null));
	}
