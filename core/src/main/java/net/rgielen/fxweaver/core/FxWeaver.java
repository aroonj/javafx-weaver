...
/**
 * FxWeaver is the core weaving facility, enabling Controllers and Views to be
 * instantiated by a dependency injection framework such as (but not limited to)
 * Spring.
 * <p/>
 * The following example requires a Spring ConfigurableApplicationContext to be
 * instantiated. If now MainController is declared as Spring managed bean, it
 * will get created and injected by Spring.
 * <p>
 * If a managed Controller class contains an {@link FxmlView} annotation,
 * attached FXML views are injected as well.
 *
 * <pre>
...
public class FxWeaver {
	 ...
	 * </pre>
	 *
	 * @param beanFactory  The beanFactory callback to be called for requesting a
	 *                     bean of given class when e.g. {@link #loadView(Class)} is
	 *                     called.
	 ...
	 * fx:controller as a bean produced by the bean factory provided in
	 * {@link #FxWeaver(Callback, Runnable)}.
	 * <p/>
	 * The possible FXML resource is inferred from a {@link FxmlView} annotation at
	 * the controller class or the simple classname and package of said class if it
	 * was not annotated like this. If the FXML file is resolvable, the defined view
	 * within will be loaded by {@link FXMLLoader}. The controller will then be
	 * instantiated based on the fx:controller attribute, using the bean factory
	 * from {@link #FxWeaver(Callback, Runnable)}. If the bean factory is based on a
	 * dependency management framework such as Spring, Guice or CDI, this means that
	 * the instance will be fully managed and injected as declared.
	 * <p/>
	 * If you are interested in the controller instance, you might instead use the
	 * <tt>loadController</tt> methods, e.g. {@link #loadController(Class)}
	 *
	 * @param controllerClass The controller class of which a weaved instance should
	 *                        be provided
	 * @param <C>             The controller type
	 * @param <V>             The view type
	 * @return An instance of the requested view, weaved with its managed controller
	 *         as defined in {@link FXMLLoader#getController()}.
	 ...
	 * {@link #FxWeaver(Callback, Runnable)}.
	 * <p/>
	 * The possible FXML resource may be given as a location in the classpath. If
	 * the FXML file is resolvable, the defined view within will be loaded by
	 * {@link FXMLLoader}. The controller will then be instantiated based on the
	 * fx:controller attribute, using the bean factory from
	 * {@link #FxWeaver(Callback, Runnable)}. If the bean factory is based on a
	 * dependency management framework such as Spring, Guice or CDI, this means that
	 * the instance will be fully managed and injected as declared.
	 * <p/>
	 * If you are interested in the controller instance, you might instead use the
	...
	public <C, V extends Node> V loadView(Class<C> controllerClass, String location, ResourceBundle resourceBundle) {
		return this.<C, V>load(controllerClass, location, resourceBundle).getView().orElse(null);
	}
	 ...
	 *                        be provided
	 * @param location        The location of the FXML view to load as a classloader
	 *                        resource. May be <tt>null</tt> or not resolvable, in
	 *                        which case the controller will be directly
	 *                        instantiated by the given bean factory.
	 * @param <C>             The controller type
	 * @return A managed instance of the requested controller, potentially weaved
	 *         with its view
	 * @see #FxWeaver(Callback, Runnable)
	 * @see FXMLLoader
	 */
	 ...
	 * @see FXMLLoader
	 */
	protected <C, V extends Node> FxControllerAndView<C, V> load(Class<C> controllerClass, String location,
			ResourceBundle resourceBundle) {
		return Optional.ofNullable(location).map(controllerClass::getResource)
	 ...
	 *          {@link FxmlView} annotation to specify resource to load, it is
	 *          assumed that the view resides in the same package, named
	...
	protected String buildFxmlReference(Class<?> c) {
		return Optional.ofNullable(c.getAnnotation(FxmlView.class)).map(FxmlView::value)
				.map(s -> s.isEmpty() ? null : s).orElse(c.getSimpleName() + ".fxml");
	}
